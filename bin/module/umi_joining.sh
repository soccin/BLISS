...
if [[ -z "$cutsite" && $numb_of_files == 1 ]]; then # IF THERE IS NO ENZYME && WITH SE READS
    ...
    wait $pid1
    wait $pid2
    cat $aux/forward $aux/reverse | LC_ALL=C sort --parallel=8 --temporary-directory=$SCRATCH_DIR -k1,1 > $aux/id.chr.loc.strand & pid1=$!
    cat $out/filtered.r1.fa | tr -d ">" | cut -d':' -f-7 | paste - - > $aux/id.umi.barcode.genomic_aux & pid2=$!
    wait $pid1
    wait $pid2

    cat $aux/id.umi.barcode.genomic_aux | awk '{if (NF == 4) print; else if (NF == 5) print $1,$3,$4,$5}' | LC_ALL=C sort --parallel=8 --temporary-directory=$SCRATCH_DIR -k1,1 > $aux/id.umi.barcode.genomic

    LC_ALL=C join $aux/id.chr.loc.strand $aux/id.umi.barcode.genomic | cut -d' ' -f-5 |LC_ALL=C sort --parallel=8 --temporary-directory=$SCRATCH_DIR -t' ' -k2,2 -k3,3n -k4,4 -k5,5 |
